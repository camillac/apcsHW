...
public class Sorts {
    ...
    private void bubbleSort_small() {
        for (Comparable index: data) {
			for (int traverse = 0; traverse < data.length - 1; traverse++) {
				Comparable current = data[traverse];
				if (data[traverse].compareTo(data[traverse + 1]) > 0) {
					data[traverse] = data[traverse + 1];
					data[traverse + 1] = current;
				}
			}
		}
	}
