...
public class OrderedArrayList_String  {
	...
	private int recurseIndex(int lowest, int index, int highest, String findMe) {
		...
		// index given lower than index we want to find
			...
			return recurseIndex(index, (highest + index)/2, highest, findMe);
		
		// index given is higher than index we want
		else if (arrayList.get(index).compareTo(findMe) > 0){
			return recurseIndex(lowest, (lowest + index)/2, index, findMe);}
		else
			return -1;
	}
   ...
   /** 
     ...
     */
     private int champIndex( ArrayList<String> challengers) {
		 ...
		 for (int i = 0; i < challengers.size(); i++) {
			 if (champS.compareTo(challengers.get(i)) > 0)  {
				 champ = i;
    ...
    public OrderedArrayList_String( ArrayList<String> unordered) {
		...
		ArrayList<String> meow = unordered;
        String x = "[";  // placeholder, awaiting replacement
		for (int i = 0; i < meow.size(); i++ ) {
			System.out.println(x);
			x += meow.get( nextLargerAt) + ", ";
			System.out.println(meow.get( nextLargerAt));
			System.out.println(nextLargerAt);
			meow.set(nextLargerAt, null);
			System.out.println(meow);
			//System.out.println(champIndex(unordered) + "Kill yourse");
			nextLargerAt = champIndex( meow);
			System.out.println(nextLargerAt + "mw");
		}
		x += "]";
		System.out.println(x);
    }
